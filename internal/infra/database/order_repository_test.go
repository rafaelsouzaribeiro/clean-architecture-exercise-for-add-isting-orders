...
func (suite *OrderRepositoryTestSuite) SetupSuite() {
	...
	db.Exec("INSERT INTO orders(id,price,tax,final_price)VALUES('123', 10.0, 2.0, 12.0), ('456', 20.0, 4.0, 24.0)")
...
func (suite *OrderRepositoryTestSuite) TestGivenOrders_WhenGetAll_ThenShouldReturnAllOrders() {
	rows, err := suite.Db.Query("SELECT id, price, tax, final_price FROM orders")
	suite.NoError(err)
	defer rows.Close()
	...
	suite.Equal(orders[0].ID, "123")
	suite.Equal(orders[0].Price, 10.0)
	suite.Equal(orders[0].Tax, 2.0)
	suite.Equal(orders[0].FinalPrice, 12.0)

	suite.Equal(orders[1].ID, "456")
	suite.Equal(orders[1].Price, 20.0)
	suite.Equal(orders[1].Tax, 4.0)
	suite.Equal(orders[1].FinalPrice, 24.0)
	suite.Len(orders, 2)
}
